...
router.put("/update-attendance/:id", async (req, res) => {
  try {
    const { checkOutTime, checkOutLocationName, contactNumber, calltime } = req.body;

    // Ensure all required fields are present
    if (!checkOutTime || !checkOutLocationName || !contactNumber || calltime) {
      return res.status(400).json({
        message:
          "Check-out time, location, contact number, and calltime array must be provided.",
      });
    }
    ...
    // Add the new check-out record, including calltime
    attendance.checkOutRecords.push({
      checkOutTime,
      checkOutLocationName,
      contactNumber,
